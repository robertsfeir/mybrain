...
async function runMigrations(pool, migrationsDirOverride) {
  ...
  try {
    ...
    // -------- Fresh-install baseline bootstrap --------
    //
    // The migration files in migrations/ assume the merged baseline schema
    // (types like thought_type, tables like thoughts) already exists -- they
    // ALTER and extend, they do not create from scratch. On a truly empty
    // database, migration 001 fails on its first `ALTER TYPE thought_type
    // ADD VALUE` because the type does not exist. The .mcpb / Desktop
    // Extension install path lands users in exactly this state.
    //
    // Detect "fresh DB" as "no public.thoughts table" and apply
    // templates/schema.sql before the migration loop. The baseline is
    // additive (CREATE TYPE / CREATE TABLE without DROP); we never run it
    // on a DB that already has thoughts. Migrations 001-005 are all
    // idempotent on top of the merged baseline (CREATE TABLE IF NOT
    // EXISTS, ALTER TYPE ADD VALUE IF NOT EXISTS, ON CONFLICT DO
    // NOTHING), so they cleanly no-op after bootstrap.
    // Use to_regclass to respect the current search_path -- tests run in
    // isolated schemas (mybrain_test_migration, mybrain_hardening_*, etc.),
    // and a hardcoded `table_schema = 'public'` filter would falsely flag
    // those as fresh on every test run.
    const baselineRes = await client.query(`SELECT to_regclass('thoughts') AS oid`);
    if (baselineRes.rows[0].oid === null) {
      const baselinePath = path.join(repoRoot, "templates", "schema.sql");
      if (!existsSync(baselinePath)) {
        throw new Error(
          `Fresh-install bootstrap failed: templates/schema.sql not found at ${baselinePath}. ` +
          `An empty database needs the baseline schema before migrations can apply.`
        );
      }
      let baseline = readFileSync(baselinePath, "utf-8");
      // Default to 1536 dims (OpenAI text-embedding-3-small, the default
      // embedding provider). Operators using a different-dim model must
      // apply schema.sql by hand before first startup with the right
      // {{EMBED_DIM}} substitution; we cannot probe the model from here.
      const embedDim = "1536";
      baseline = baseline.replaceAll("{{EMBED_DIM}}", embedDim);
      console.error(
        `Migration runner: empty database detected, applying templates/schema.sql baseline (EMBED_DIM=${embedDim})...`
      );
      await client.query(baseline);
      const checksum = createHash("sha256").update(baseline).digest("hex").slice(0, 16);
      await client.query(
        `INSERT INTO schema_migrations (version, applied_at, checksum)
         VALUES ($1, now(), $2)
         ON CONFLICT (version) DO NOTHING`,
        ["000-baseline-schema.sql", checksum]
      );
      console.error("Migration runner: baseline schema applied.");
    }
