...
// Module-scope shared `failedIds` Map (id -> attempt count). Shared between
// startEmbedWorker (background tick loop) and flushEmbedQueue (synchronous
// drain called by agent_search in async-storage mode). Sharing the Map means
// flushEmbedQueue respects the worker's already-accumulated failure counts
// and won't retry rows the worker has permanently abandoned. Map preserves
// insertion order, so eviction of the oldest entry is O(1).
const failedIds = new Map();

// Process a single row: generate its embedding and UPDATE the row.
// Tracks attempts in a shared `failedIds` Map (id -> attempt count) so a
// permanently-failing row can't starve the queue across repeated drains.
async function processEmbedRow(pool, embedProviderConfig, row, label) {
  const attempts = failedIds.get(row.id) || 0;
  if (attempts >= WORKER_MAX_ATTEMPTS) return false;
  try {
    const vec = await getEmbedding(row.content, embedProviderConfig);
    await pool.query(
      `UPDATE thoughts SET embedding = $1 WHERE id = $2 AND embedding IS NULL`,
      [pgvector.toSql(vec), row.id]
    );
    if (failedIds.has(row.id)) failedIds.delete(row.id);
    return true;
  } catch (err) {
    const next = attempts + 1;
    if (!failedIds.has(row.id) && failedIds.size >= FAILED_IDS_CAP) {
      const oldest = failedIds.keys().next().value;
      if (oldest !== undefined) failedIds.delete(oldest);
    }
    failedIds.set(row.id, next);
    if (next >= WORKER_MAX_ATTEMPTS) {
      console.error(`${label}: row ${row.id} permanently failed after ${WORKER_MAX_ATTEMPTS} attempts — skipping (last error: ${err.message})`);
    } else {
      console.error(`${label}: row ${row.id} failed (attempt ${next}/${WORKER_MAX_ATTEMPTS}):`, err.message);
    }
    return false;
  }
}
...
// =============================================================================
// flushEmbedQueue(pool, embedProviderConfig) — ADR-0002
// =============================================================================
//
// Drain all `thoughts` rows WHERE embedding IS NULL to completion. Used by
// agent_search in async-storage mode to eliminate the capture-then-search race:
// a thought captured ~500ms ago may not have been picked up by the background
// worker yet, so search would miss it. Calling this in parallel with query
// embedding hides the flush cost under the embedding round-trip in the common
// case (small queue: 1-8 rows; query embed: ~1s).
//
// Single-user assumption: no concurrent writer is racing rows in behind us.
// "Drain to empty" is therefore well-defined.
//
// Self-bounded: each row tracked in the module-shared failedIds Map;
// permanently-failing rows are skipped after WORKER_MAX_ATTEMPTS retries.
// The Map is shared with startEmbedWorker, so rows the background worker has
// already abandoned are not re-attempted here. The outer loop terminates when
// a batch returns rows but no row succeeded (every remaining row is permanently
// failed) or when the queue returns zero rows.

async function flushEmbedQueue(pool, embedProviderConfig) {
  const BATCH = Number(process.env.MYBRAIN_WORKER_BATCH || 8);
  // Hard cap on outer iterations: BATCH * WORKER_MAX_ATTEMPTS gives a safe
  // upper bound — even if every batch is permanent-fails being retried, we
  // can't loop more than this. In practice we exit on the first empty or
  // no-progress batch.
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    const results = await Promise.allSettled(
      rows.map((r) => processEmbedRow(pool, embedProviderConfig, r, "flushEmbedQueue"))
    );
    const anyProgress = results.some(
      (res) => res.status === "fulfilled" && res.value === true
    );
    if (!anyProgress) {
      // Every row in this batch is a permanent failure (or already maxed out
      // attempts within this call). Stop — looping again would just re-fetch
      // the same rows.
      return;
    }
  }
...
function startEmbedWorker(pool, embedProviderConfig) {
  ...
  const BATCH = Number(process.env.MYBRAIN_WORKER_BATCH || 8);
  // Uses the module-shared `failedIds` Map (declared above). Sharing with
  // flushEmbedQueue ensures a row this worker has permanently abandoned is
  // not given a fresh 5 retries by every async-mode agent_search call.
  let running = false;
  ...
  async function tick() {
    ...
    try {
      ...
      await Promise.allSettled(
        rows.map((r) => processEmbedRow(pool, embedProviderConfig, r, "embed worker"))
      );
    } catch (err) {
  ...
  console.error(`embed worker started (poll ${POLL_MS}ms, batch ${BATCH}, max-attempts ${WORKER_MAX_ATTEMPTS})`);
  return handle;
}
...
export { getEmbedding, probeEmbeddingDim, startEmbedWorker, flushEmbedQueue };
