...
function resolveConfig() {
  ...
  const cwdPath = cwd + "/.claude/brain-config.json";
  const userPath = process.env.BRAIN_CONFIG_USER;
  const homePath = os.homedir() + "/.claude/brain-config.json";

  // .mcpb / Desktop Extension hostile-cwd guard. Claude Desktop launches
  // extensions with cwd set to the extension folder
  // (~/Library/Application Support/Claude/Claude Extensions/local.mcpb.<id>/).
  // Any leftover or template `.claude/brain-config.json` inside the
  // extension folder would otherwise shadow the user's real config in
  // ~/.claude/brain-config.json -- as it did for Frank on 2026-05-07,
  // where a partial config (no openrouter_api_key) silently shadowed the
  // working one and killed the server at server.mjs:78. Skipping the cwd
  // candidate when cwd looks like an extension folder forces resolution
  // to fall through to BRAIN_CONFIG_USER / ~/.claude/brain-config.json.
  // The check covers both POSIX (macOS / Linux) and Windows path forms.
  const skipCwd =
    cwd.includes("/Claude Extensions/") || cwd.includes("\\Claude Extensions\\");
  if (skipCwd) {
    console.error(
      `Config resolution: cwd is inside a Claude Desktop extension folder (${cwd}); ` +
      `skipping cwd config lookup to avoid shadowing ~/.claude/brain-config.json.`
    );
  }

  const candidates = [
    projectPath,
    skipCwd ? null : cwdPath,
    userPath,
    homePath,
  ];
  ...
  for (const configPath of candidates) {
    ...
    try {
      ...
      if (skipCwd && configPath === homePath) {
        resolved._source = "personal-home-extension-cwd-skipped";
      }
      return resolved;
    } catch {
      continue;
  ...
  //
  // v2.2.4: extended to also synthesize an Ollama provider config from
  // EMBEDDING_PROVIDER / OLLAMA_HOST / OLLAMA_MODEL env vars when set,
  // so .mcpb users can pick Ollama via the install dialog without
  // hand-writing a brain-config.json.
  const dbUrl = process.env.DATABASE_URL || process.env.ATELIER_BRAIN_DATABASE_URL;
  const apiKey = process.env.OPENROUTER_API_KEY;
  const provider = (process.env.EMBEDDING_PROVIDER || "").toLowerCase();
  if (dbUrl) {
    const cfg = { database_url: dbUrl, openrouter_api_key: apiKey, _source: "env" };
    if (provider === "ollama" || provider === "local") {
      cfg.embedding_provider = "local";
      cfg.chat_provider = "local";
      const ollamaHost = process.env.OLLAMA_HOST || "http://localhost:11434";
      const baseUrl = ollamaHost.replace(/\/v1\/?$/, "") + "/v1";
      cfg.embedding_base_url = baseUrl;
      cfg.chat_base_url = baseUrl;
      if (process.env.OLLAMA_MODEL) cfg.embedding_model = process.env.OLLAMA_MODEL;
    }
    return cfg;
  }
