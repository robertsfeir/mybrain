...
function registerAgentCapture(srv, pool, apiKey, capturedBy, cfg, chatKey) {
  srv.tool(
    "agent_capture",
    "Store a thought with schema-enforced metadata. Handles dedup, conflict detection, and supersedes relations. Pass 'relations' to link the new thought to existing thoughts in the same call (preferred over a follow-up atelier_relation call). Required: content, thought_type, source_agent, source_phase, importance.",
    {
      ...
      supersedes_id: z.string().uuid().optional().describe("UUID of thought this supersedes (legacy; prefer 'relations' with relation_type='supersedes')"),
      ...
      relations: z.array(z.object({
        target_id: z.string().uuid().describe("UUID of the existing thought to link to"),
        ...
        "Inline relations to create alongside this capture. Each entry links the new thought (source) to an existing thought (target) with a typed edge. All relation inserts share the same transaction as the thought insert; if any relation fails, the entire capture rolls back. Avoids the second atelier_relation round-trip for the common 'capture + relate' pattern."
      ),
    },
...
async function handleAgentCapture(params, pool, apiKey, capturedBy, cfg, chatKey) {
  ...
  const { content, thought_type, source_agent, source_phase, importance,
          ...
          decided_by, alternatives_rejected, evidence, confidence,
          relations = [] } = params;
  ...
  // mybrain ADR-0001 (inline relations): reject the ambiguous combination of
  // legacy 'supersedes_id' + an inline 'supersedes' relation. The two fields
  // could name different target_ids; silently letting one win produces
  // hard-to-debug captures.
  if (supersedes_id && relations.some(r => r.relation_type === "supersedes")) {
    return {
      content: [{
        type: "text",
        text:
          `Error: agent_capture received both 'supersedes_id' and a 'supersedes' entry in 'relations'. ` +
          `Pick one: 'relations' (preferred) or 'supersedes_id' (legacy), not both — they could name different target_ids.`,
      }],
      isError: true,
    };
  }

  // Dedup within relations[]: the DB UNIQUE (source_id, target_id, relation_type)
  // would coalesce duplicates via ON CONFLICT, but failing fast at the API
  // boundary is more honest than silent collapse.
    ...
    for (const rel of relations) {
      ...
      if (seen.has(key)) {
        return {
          content: [{
            type: "text",
            text: `Error: duplicate entry in 'relations': target_id=${rel.target_id} relation_type=${rel.relation_type}.`,
          }],
          isError: true,
        };
      }
  ...
  try {
    ...
    if (conflictResult.action === "merge") {
      return await handleMerge(client, content, importance, enrichedMetadata, conflictResult, capturedBy, relations);
    }
    ...
    await handleInlineRelations(client, newThought.id, relations, relatedIds);
...
async function handleMerge(client, content, importance, metadata, conflictResult, capturedBy, relations = []) {
  await client.query(
    `UPDATE thoughts SET
  ...
  // mybrain ADR-0001: relations[] target the new thought; the merge path
  // doesn't insert one, so dropping them silently would lose data invisibly.
  if (relations.length > 0) {
    payload.warning = `agent_capture relations[] ignored: capture was merged into existing thought ${conflictResult.existingId}. Use atelier_relation against thought_id if you need explicit links on the merged row.`;
  }
  return {
...
async function handleInlineRelations(client, newId, relations, relatedIds) {
  if (!relations || relations.length === 0) return;

  // One round-trip to verify all targets exist; FK violations would still be
  // caught downstream, but the message would name the constraint, not the id.
  const targetIds = [...new Set(relations.map(r => r.target_id))];
  const existsRes = await client.query(
    `SELECT id FROM thoughts WHERE id = ANY($1::uuid[])`,
    [targetIds]
  );
  const found = new Set(existsRes.rows.map(r => r.id));
  const missing = targetIds.filter(id => !found.has(id));
  if (missing.length > 0) {
    throw new Error(
      `relations[] references unknown thought id(s): ${missing.join(", ")}. ` +
      `All target_ids must reference existing thoughts in the same database.`
    );
  }

  for (const rel of relations) {
    if (rel.target_id === newId) {
      // Defensive: agent_capture generates a fresh UUID, so a self-relation
      // can only happen via deliberate UUID forging. Reject loudly.
      throw new Error(`relations[] entry would create a self-referential edge (target_id == new thought id)`);
    }
    if (rel.relation_type === "supersedes") {
      const hasCycle = await checkSupersedeCycle(client, newId, rel.target_id);
      if (hasCycle) {
        throw new Error(`relations[] supersedes entry would create a cycle (target_id=${rel.target_id})`);
      }
    }
    await client.query(
      `INSERT INTO thought_relations (source_id, target_id, relation_type, context)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (source_id, target_id, relation_type) DO UPDATE SET context = EXCLUDED.context`,
      [newId, rel.target_id, rel.relation_type, rel.context ?? null]
    );
    if (rel.relation_type === "supersedes") {
      await client.query(
