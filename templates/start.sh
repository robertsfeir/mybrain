...
EOSQL
  ...
  log "Applying schema (EMBED_DIM=${EMBED_DIM})..."
  sed "s/{{EMBED_DIM}}/${EMBED_DIM}/g" /app/schema.sql | gosu postgres psql -v ON_ERROR_STOP=1 -U mybrain -d mybrain -f -
...
if ! ollama list 2>/dev/null | grep -q "${OLLAMA_MODEL}"; then
  log "Pulling ${OLLAMA_MODEL} (first boot only — this may take a few minutes)..."
