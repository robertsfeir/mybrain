...
function registerTools(srv) {
  srv.tool(
    ...
    async ({ content, metadata = {} }) => {
      ...
      const result = await pool.query(
        `INSERT INTO thoughts (content, embedding, metadata) VALUES ($1, $2, $3) RETURNING id, created_at`,
  ...
  srv.tool(
    ...
    async ({ limit = 20, offset = 0, filter = {} }) => {
      ...
      const result = await pool.query(
        `SELECT id, content, metadata, created_at FROM thoughts ${hasFilter ? "WHERE metadata @> $3" : ""} ORDER BY created_at DESC LIMIT $1 OFFSET $2`,
        hasFilter ? [limit, offset, JSON.stringify(filter)] : [limit, offset]
      );
  ...
  srv.tool(
    ...
    {},
    async () => {
      const countRes = await pool.query("SELECT count(*) as total FROM thoughts");
      const dateRes = await pool.query("SELECT min(created_at) as earliest, max(created_at) as latest FROM thoughts");
      const metaRes = await pool.query(`SELECT key, count(*) as cnt FROM thoughts, jsonb_each(metadata) AS kv(key, value) GROUP BY key ORDER BY cnt DESC LIMIT 10`);
